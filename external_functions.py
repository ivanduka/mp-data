        ...
        try:
            ...
            for index, row in df_tables.iterrows():
                ...
                if prev_id > 0:
                    ...
                    page_list = table_checker(doc_text, doc_text_rotated, doc_id, toc_id, toc_page, s1_rex, s2_rex)
                    if len(page_list) > 0:
                        id_list = [doc_id]
                        count = len(page_list)
                ...
                if count == 0:
                    for doc_id, doc in df_project.iterrows():
                        if (doc_id != toc_id) and (doc_id != prev_id):
                            doc_text = df_project.loc[doc_id, 'Text']
                            doc_text_rotated = df_project.loc[doc_id, 'Text_rotated']
                            p_list = table_checker(doc_text, doc_text_rotated, doc_id, toc_id, toc_page, s1_rex, s2_rex)
                            if len(p_list) > 0:
